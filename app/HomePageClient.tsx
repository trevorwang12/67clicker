...
export default function HomePageClient({ initialSeoData }: HomePageClientProps) {
  ...
  useEffect(() => {
    ...
    const title = seoData?.seoSettings?.siteName
    if (title) {
  ...
  useEffect(() => {
    ...
    const loadSEOData = async (force = false) => {
      if (!force && initialSeoData) return
      try {
